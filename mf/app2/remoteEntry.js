...
(() => {
		...
		'webpack/container/entry/app2': (__unused_webpack_module, exports, __webpack_require__) => {
			var moduleMap = {
				'./Button': () => {
					...
					]).then(() => () => __webpack_require__(/*! ./src/Button.js */ './src/Button.js'));
				},
			};
	...
	(() => {
		...
		__webpack_require__.l = (url, done, key, chunkId) => {
			...
			console.log('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ app2 添加一个宏任务：🚩 start load chunk by script', url);
			...
			var onScriptComplete = (prev, event) => {
				console.error(
					'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ app2 加载完成个宏任务：🚩 end load chunk by script',
					url
				);
	...
	(() => {
		...
		__webpack_require__.I = (name, initScope) => {
			console.log(
				'第',
				++count,
				'几次调用 app2 remoteEntry __webpack_require__.I，当前 加载 name is',
				name,
		...
		var webpackJsonpCallback = (parentChunkLoadingFunction, data) => {
			...
			for (; i < chunkIds.length; i++) {
				...
				if (__webpack_require__.o(installedChunks, chunkId) && installedChunks[chunkId]) {
					console.log('success, 😄 app2 - chunkId 完成加载 resolve', chunkId);

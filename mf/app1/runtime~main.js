(() => {
	...
	(() => {
		...
		__webpack_require__.e = chunkId => {
			console.log('\r\n');
			console.warn('------- 🚩 开始 __webpack_require__.e 同步阶段，收集 promises', chunkId);
			// Object.keys(__webpack_require__.f) = ['remotes', 'consumes', 'j']
			const res = Promise.all(
				...
				console.log('------ ✅ 完成 __webpack_require__.e 异步节点 promise.then', chunkId);
				return val;
			});
			console.warn('------ 结束 __webpack_require__.e 同步阶段，收集 promises', chunkId);
			console.log('\r\n');
			return res;
		};
	})();
	...
	(() => {
		...
		__webpack_require__.l = (url, done, key, chunkId) => {
			...
			console.log('****************************** app1 添加一个宏任务：🚩 start load chunk by script', url);
			...
			var onScriptComplete = (prev, event) => {
				console.error('****************************** app1 加载完成个宏任务：🚩 end load chunk by script', url);
	...
	(() => {
		var chunkMapping = {
			src_bootstrap_js: ['webpack/container/remote/app2/Button'],
		};
		var idToExternalAndNameMapping = {
			'webpack/container/remote/app2/Button': ['default', './Button', 'webpack/container/reference/app2'],
		};
		__webpack_require__.f.remotes = (chunkId, promises) => {
			if (__webpack_require__.o(chunkMapping, chunkId)) {
				chunkMapping[chunkId].forEach(id => {
					console.log('开始执行 __webpack_require__.f.remotes', chunkId, id);
					...
					var handleFunction = (fn, arg1, arg2, d, next, first) => {
						try {
							if (first) {
								console.log(
									`remotes 第一次：__webpack_require__('webpack/container/reference/app2')，返回 promise<app2>`
								);
							} else if (arg1 === data[0]) {
								console.log('执行 handleFunction-onExternal: __webpack_require__.I("default"), 更新 __webpack_require__.S');
							} else if (arg1 === data[1]) {
								console.log('执行 handleFunction-onInitialized: app2.get("./Button")');
							} else {
								console.log(
									'执行 handleFunction-onFactory: app2.get("./Button") 获取 webpack/container/remote/app2/Button 模块 export 内容'
								);
							}
							var promise = fn(arg1, arg2);
							if (next) {
								console.log('\t\t  remotes next is', next.name, !!(promise && promise.then));
							}
	...
	(() => {
		...
		__webpack_require__.I = (name, initScope) => {
			console.log(
				'第',
				++count,
				'几次调用 app1 runtime __webpack_require__.I，当前 加载 name is',
				name,
				JSON.stringify(__webpack_require__.S, null, 4)
			);
			...
			console.log('~~~~~~~~~~~~~ ⏰ 进行 app1 __webpack_require__.S 初始化 ~~~~~~~~~~~~~~~~~~~~~');
	...
	(() => {
		...
		var get = entry => {
			entry.loaded = 1;
			console.log('app1 load', entry.get);
			return entry.get();
		};
		var init = fn =>
		...
		var loadStrictVersionCheckFallback = init((scopeName, scope, key, version, fallback) => {
			var entry = scope && __webpack_require__.o(scope, key) && findValidVersion(scope, key, version);
			return entry ? get(entry) : fallback();
		});
		var loadStrictSingletonVersionCheckFallback = init((scopeName, scope, key, version, fallback) => {
			if (!scope || !__webpack_require__.o(scope, key)) return fallback();
			return getStrictSingletonVersion(scope, scopeName, key, version);
		});
		...
		var chunkMapping = {
			'common-webpack_sharing_consume_default_react_react': ['webpack/sharing/consume/default/react/react'],
		...
		__webpack_require__.f.consumes = (chunkId, promises) => {
			if (__webpack_require__.o(chunkMapping, chunkId)) {
				chunkMapping[chunkId].forEach(id => {
					...
					try {
						console.log('__webpack_require__.f.consumes', 'chunkid', chunkId, id);
		...
		__webpack_require__.f.j = (chunkId, promises) => {
			...
			if (installedChunkData !== 0) {
				...
				if (installedChunkData) {
					...
					if (!/^(common\-webpack_sharing_consume_default_react_react|runtime~main)$/.test(chunkId)) {
